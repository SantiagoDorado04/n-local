...
class OnlineRegistrationFormOptionsComponent extends Component
{
    ...
    {
        $this->questionId = $id;
        $this->question = OnlineRegistrationFormQuestion::find($this->questionId);

        if ($this->question && $this->question->onlineRegistrationForm) {
            $courseId = $this->question->onlineRegistrationForm->onlineRegistrationCourse->id;
    ...
    public function render()
    {
        ...
        $options = OnlineRegistrationFormOption::when($this->searchName, function ($query, $searchName) {
        ...
        return view('livewire.admin.online-registration-forms.online-registration-form-options-component', [
            'options' => $options,
    ...
    public function show($id)
    {
        ...
        $option = OnlineRegistrationFormOption::find($id);
    ...
    public function store()
    {
        $this->validate([
            'text' => [
                ...
                function ($attribute, $value, $fail) {
                    $exists = OnlineRegistrationFormOption::where('question_id', $this->questionId)
                        ->where('text', $value)
                        ->exists();
                    if ($exists) {
        ...
        if ($this->position == '') {
            $maxPosition = OnlineRegistrationFormOption::where('question_id', $this->questionId)->max('position');
            $this->position = is_null($maxPosition) ? 1 : $maxPosition + 1;
        }

        $option = new OnlineRegistrationFormOption();
    ...
    public function update()
    {
        ...
        $this->validate([
            'text' => [
                ...
                function ($attribute, $value, $fail) {
                    if ($this->optionId) {
                        $exists = OnlineRegistrationFormOption::where('question_id', $this->questionId)
                            ->where('text', $value)
                            ->where('id', '!=', $this->optionId) // Ignora la opción actual
                            ->exists();
                        if ($exists) {
            ...
            'value' => 'required',
            'position' => 'nullable|integer', // Se eliminó "sometimes" porque "nullable" ya cubre el caso
            'conditional' => 'required',
        ], [], [
            'text' => 'texto',
            'value' => 'valor',
            'position' => 'posición', // Se corrigió el acento
            'conditional' => 'condicional',
        ]);


        $option = OnlineRegistrationFormOption::find($this->optionId);
        $option->text = $this->text;
        $option->value = $this->value;
        $option->position = $this->position;
        $option->conditional = $this->conditional;
    ...
    public function updateOptionOrder($orderedIds)
    {
        foreach ($orderedIds as $index => $id) {
            $options = OnlineRegistrationFormOption::find($id);
